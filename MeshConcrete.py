...
class Solves(object):
    ...
    def centerMass(lst):
        '''Состав lstForm:
            ['rectangle/solidcircle/circle/triangle',h,b,dx,dy, nx, ny, nummat, typmat]'''
        a=0
        sx=0
        sy=0
        for i in lst:
            if i[0]=='rectangle':
                a+=i[1]*i[2]
                sx+=i[1]*i[2]*(i[3]-i[2]/2)
                sy+=i[1]*i[2]*(i[4]-i[1]/2)
            elif i[0]=='solidcircle':
                a+=(i[1]/2.)**2*3.14
                sx+=(i[1]/2.)**2*3.14*(i[3]-i[1]/2)
                sy+=(i[1]/2.)**2*3.14*(i[4]-i[1]/2)
            elif i[0]=='triangle':
                a+=1./2.*abs((x1-x3)*(y2-y3)-(x2-x3)*(y1-y3))
                sx+=1./2.*abs((x1-x3)*(y2-y3)-(x2-x3)*(y1-y3))*(x1+x2+x3)/3.
                sy+=1./2.*abs((x1-x3)*(y2-y3)-(x2-x3)*(y1-y3))*(y1+y2+y3)/3.
...
class Circles(object):
    def __init__(self,d,x,y, mat=1, sign=1):
        self.d=float(d)
